...
with open(sys.argv[1]) as tpl_file:
    ...
    for full_line in tpl_file:
        ...
        if ignore_text:
            ...
            if end_table:
                endtable_count, table_eval = tplser.sectionparse.close_match(
                    endtable_count, table_eval)
                table_eval, table_err = tplser.evaluations.loop_eval(table_eval, table_err, line_num)
            ...
            if re.match(log_rx, line):
                ...
                if "+" in no_quotes:
                    if "time." in no_quotes:  # time function is not a string
                        eca_errs.append(
                            str(line_num) + ", Concatenation of strings in log statement:\n      " + str.strip(line))
            ...
            if re.search(end_identify_rx, line):
                endsid_count, sid_eval, missing_sid, sid_err, missing_sids, sids_err \
                    = tplser.sectionparse.close_requireds(pattern_name, line, endsid_count, sid_eval, missing_sid,
                                                          sid_err, sid_tags, missing_sids, sids_err)
            ...
            pattern_name, pattern_num, endpattern_num, patt_eval, pattern_parsing, patt_err, pattern_list, varlist = \
                tplser.sectionparse.pattern_parse(pattern_name, fwd, line, pattern_num, endpattern_num, patt_eval,
        ...
        if tpl_parsing:
            ...
            if re.match(end_if_rx, line):
                endif_count, if_eval = tplser.sectionparse.close_match(endif_count, if_eval)
                if_eval, if_err = tplser.evaluations.loop_eval(if_eval, if_err, line_num)
            ...
            if re.match(end_for_rx, line):
                endfor_count, for_eval = tplser.sectionparse.close_match(endfor_count, for_eval)
                for_eval, for_err = tplser.evaluations.loop_eval(for_eval, for_err, line_num)

            # Check for syntax errors with the 'stop;' statement
            if if_eval > 0 and "stop;" in line:
                stop_pattern = True

            if stop_pattern and "stop;" not in line:
                ...
                if not stopright:
                    if line.isspace():  # if line is just whitespace ignore
                        ...
                        syntax_errs.append(
                            str(line_num) + ", statement not reachable after stop:\n      " + str.strip(line))
                stop_pattern = False
            ...
            if var_declared:
                ...
                if in_brackets:
                    vas = re.findall(in_bracket_vars, in_brackets.group(1))
                    for var in vas:
                    ...
                    if re.search(model_rx, var_line):
                        ...
                        vas = re.findall(embed_vars, embed_line)
                        for var in vas:
                            if var == "text":
                                pass  # text function
                            elif var == "not":
                ...
                if var_plus:
                    ...
                    if var == "text":
                        if "%" in line:
                            vas = re.findall(sub_rx, var_line)
                            for var in vas:
                                if "[" in var or "." in var:
                                    used.append(re.search(sp_chars_rx, var).group(1))
                                else:
                ...
                if if_line:
                    ...
                    if "." in var:
                        ...
                        vas = re.findall(if_text_rx, var_line)
                        for var in vas:
                            used.append(var)
                    else:
                        used.append(var)
                    ...
                    if "(" in var_line or ")" in var_line:
                        ...
                        if not openb == closeb:
                            syntax_errs.append(
                                str(line_num) + ", Uneven opening and closing brackets:\n      " + str.strip(line))
                    ...
                    if re.match(numeric_rx, var):
                        ...
                        if line_conf:
                            if line_conf.group(1) in global_vars:
                                ...
                                syntax_errs.append(
                                    str(line_num) + ", Config item not valid:\n      " + str.strip(var_line))
                        else:
                            pass
    ...
    for row in lines:
        ...
        if not ignore_text:
            ...
            if end_table:
                rev_endtable_count, rev_table_eval = tplser.sectionparse.open_match(
                    rev_endtable_count, rev_table_eval)

            table = re.search(table_rx, row)
            if table:
                rev_table_count, rev_table_eval = tplser.sectionparse.close_match(
                    rev_table_count, rev_table_eval)
                rev_table_eval, rev_table_err = tplser.evaluations.loop_eval(
                    rev_table_eval, rev_table_err, rev_line_num)

            # Pattern evaluation
            (pattern_name, rev_pattern_num, rev_endpattern_num, rev_patt_eval, rev_patt_parse, rev_patt_err,
             pattern_list, varlist) = tplser.sectionparse.pattern_parse(
                pattern_name, fwd, row, rev_pattern_num, rev_endpattern_num, rev_patt_eval, rev_patt_parse,
                rev_patt_err, rev_line_num, varlist)
        ...
        if rev_patt_parse:
            ...
            # Body evaluation
            rev_body_num, rev_endbody_num, rev_body_eval, rev_parsing, rev_body_err = tplser.sectionparse.body_parse(
            ...
            if rev_parsing:
                ...
                if re.match(end_if_rx, row):
                    rev_endif_count, rev_if_eval = tplser.sectionparse.open_match(
                        rev_endif_count, rev_if_eval)
                if re.match(if_rx, row):
                    rev_if_count, rev_if_eval = tplser.sectionparse.close_match(
                        rev_if_count, rev_if_eval)
                    rev_if_eval, rev_if_err = tplser.evaluations.loop_eval(
                        rev_if_eval, rev_if_err, rev_line_num)
                # Check FOR evaluations
                if re.match(end_for_rx, row):
                    rev_endfor_count, rev_for_eval = tplser.sectionparse.open_match(
                        rev_endfor_count, rev_for_eval)
                if re.match(for_rx, row):
                    rev_for_count, rev_for_eval = tplser.sectionparse.close_match(
                        rev_for_count, rev_for_eval)
                    rev_for_eval, rev_for_err = tplser.evaluations.loop_eval(
...
if module_num > 1:
    print(" Error: More than 1 module declaration in this file!")
    errors = True
    for mod_err in mod_line:
        print("    line %s" % mod_err)
elif module_num == 0:
    errors = True
    print(" Error: Something wrong with module declaration!")

tplser.evaluations.eval(meta_eval, "metadata")
tplser.evaluations.eval(config_eval, "configuration")
tplser.evaluations.eval(defins_eval, "definitions")

tplser.evaluations.missing_warn(
...
if tags_err > 0:
    print(" Error: Missing tags declaration(s)...")
    errors = True
    for pattern in missing_tags:
        print("    " + str(pattern))

if trig_on_err > 0:
    print(" Error: Missing trigger conditions...")
    errors = True
    for pattern in missing_trigon:
        print("    " + str(pattern))

if body_err or rev_body_err:
    tplser.printing.print_eval(
...
if initlist:
    tplser.evaluations.warnings(initlist, "Uninitialised variable(s) found")
    errors = True

if var_warn:
    var_warn = tplser.evaluations.uniq(var_warn)
    errors = True
    tplser.evaluations.warnings(
        var_warn, "Warning: Variable(s) in embedded if/for loops MAY be uninitialised")

if syntax_errs:
    tplser.evaluations.warnings(syntax_errs, "Syntax error in line(s) found")
    errors = True
...
if not tpl_ver:
    errors = True
    print(" Error: Something wrong with module version declaration!")
...
if not errors:
    print(" No problems found in TPL.")
